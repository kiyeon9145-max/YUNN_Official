...
### Result Page Google Form Feedback Gate 추가

Purpose:
결과 페이지를 바로 보여주지 않고, 진단 완료 후 Google Form 설문으로 이동하도록 하는 feedback gate 모달을 추가했다. Google Form 내부 질문은 변경될 수 있으므로 웹페이지는 설문 내용이 아니라 제출 검증용 `sessionId` 연결만 담당한다.
...
Main implementation:
- `#feedback-gate-modal` bottom sheet 추가
- 결과 생성 후 `showResults()`가 바로 결과를 표시하지 않고, 설문 제출 검증 전에는 feedback gate를 표시하도록 변경
- Google Form 이동 버튼은 `YUNN_FEEDBACK_FORM_URL`을 사용하며 현재 연결 대상은 `https://docs.google.com/forms/d/1ME2EU6G3LxnB6VPH6HUO7f-H850CGKt5Gs4lA1t8Wa8/viewform`
- `yunn_session_id` prefill을 위해 `YUNN_FEEDBACK_SESSION_ENTRY_ID` 설정값을 분리
- Apps Script 제출 검증을 위해 `YUNN_FEEDBACK_VERIFY_URL` 설정값을 분리
- Google Form으로 이동하기 전 진단 결과 계산에 필요한 답변을 `localStorage.yunn_pending_result_data`에 저장
- Google Form 완료 화면에서 `?returnFromSurvey=1`로 돌아오면 `verifyFeedbackAndUnlock()`이 실행되도록 구성
- 검증 URL이 아직 설정되지 않은 상태에서는 페이지가 깨지지 않고 “Verification setup needed” 안내를 표시
- `Not now, return to start`는 결과를 공개하지 않고 시작 화면으로 되돌아가도록 유지

Verification:
- `pages/survey.html` 내부 스크립트 구문 검사 통과: `scripts parse ok: 2`
- `feedback-gate-modal`, `openFeedbackSurvey`, `verifyFeedbackAndUnlock`, `YUNN_FEEDBACK_FORM_URL`, `YUNN_FEEDBACK_SESSION_ENTRY_ID`, `YUNN_FEEDBACK_VERIFY_URL`, `yunn_pending_result_data` 마커 존재 확인
- Google Form 질문 내용 변경은 웹페이지 코드에 영향이 없고, 제출 검증용 `yunn_session_id` 필드만 유지하면 되는 구조로 문서화

### Analysis Loading Duration 조정

Purpose:
진단 완료 후 설문 gate 모달이 뜨기 전 로딩 시간이 체감상 5초 이상으로 길게 느껴지는 문제를 줄였다. 기존에는 `600ms × 12회`로 최소 약 7.2초 동안 로딩 화면이 유지되었다.
...
Main implementation:
- `startAnalysis()`의 로딩 반복 횟수를 `12`에서 `4`로 변경
- 로딩 시간은 약 `2.4초`로 단축
- 기존 상태 메시지 4개는 유지해 분석 중이라는 체감만 남기고, 이후 바로 feedback gate 모달로 이어지도록 조정

Verification:
- `pages/survey.html` 내부 스크립트 구문 검사 통과 예정

### Survey Questions Markdown 추출

Purpose:
현재 `pages/survey.html`에 구현된 진단 설문 질문과 답변 목록을 다른 문서/Google Form 작업에서 바로 참고할 수 있도록 Markdown 파일로 추출했다.
...
Main implementation:
- Page 1~10의 질문, 설명, 답변, 입력 필드, 오류 문구, 업로드 문구를 Markdown으로 정리
- `Not sure` 선택 시 이어지는 Page 3-1~3-4 skin helper 질문도 별도 섹션으로 정리
- Google Form 설문 내용 변경 시 참고할 수 있는 기준 문서로 작성
...
### 필수 답변 단계 Next 버튼 상태 정리

Purpose:
진단 단계에서 답변을 선택하지 않았는데도 `Next` 버튼이 활성화되어 보이는 문제를 정리했다. Step 6을 포함해 필수 답변이 필요한 모든 진단 단계는 선택 전에는 `Next`가 비활성화되고, 필요한 답변 조건이 충족된 후에만 활성화되도록 통일했다.
...
Main implementation:
- Step 3, 3-1, 3-2, 3-3, 3-4, 4, 5, 6, 9의 primary `Next` 버튼에 초기 `disabled` 상태 적용
- `REQUIRED_STEP_INPUT_GROUPS`를 추가해 단계별 필수 입력 그룹을 한 곳에서 관리
- `updateStepActionState()`로 현재 단계의 필수 답변 여부를 계산해 버튼 활성/비활성 상태 갱신
- 직접 URL로 `?survey=1&step=6`처럼 진입해도 `goToStep()`에서 버튼 상태가 다시 계산되도록 보강
- Step 5, Step 6, Step 9도 `nextStep()` 진입 전 답변 누락 시 진행되지 않도록 검증 추가

Verification:
- localhost에서 Step 2, 3, 3-1, 3-2, 3-3, 3-4, 4, 5, 6, 7, 8, 9를 각각 직접 열어 초기 `Next` 버튼이 모두 `disabled=true`인지 확인
- Step 6 직접 진입 상태에서 선택 전 `Next` 비활성 확인
- Step 6 답변 선택 시 다음 단계로 정상 진행되는 것 확인
...
Purpose:
사용자가 제공한 `로그설계서`의 화면별 MD/HTML/Excel/이미지 기준에 맞춰 진단 시작 화면, 설문 Step 1~10, Skin helper Step 3-1~3-4, 로딩 화면, 결과 페이지의 주요 행동 로그를 `pages/survey.html`에 구현했다.
...
Main implementation:
- 공통 이벤트 함수 `trackYunnEvent(eventName, properties)` 추가
- `localStorage.yunn_analytics_events`, `window.dataLayer`, `yunn:analytics` 커스텀 이벤트로 동일 payload 적재
- `session_id`, `timestamp`, `screen`, `step`, `device_type`, `traffic_source`, `network`, `viewport` 공통 필드 자동 추가
- 랜딩 화면 view/click/scroll/time/exit 로그 추가
- Step 1 사용자 정보 입력 시작/완료, 이메일/전화번호 validation error 로그 추가
- Step 2~10의 page view, option view, select/change, next/back, disabled next click, time spent, abandon, scroll depth, friction 로그 추가
- Step 3 `Not sure` 이후 helper flow인 3-1~3-4도 별도 이벤트명으로 추적
- Step 10 사진 업로드 click/start/success/fail/cancel/quality/multiple attempts 로그 추가
- 결과 페이지 `result_page_view`, 피부 타입 섹션, Skin Balance 섹션, 루틴 탭, 제품 추천, unlock CTA, 장바구니, 재진단 로그 추가
- raw phone/email/image 데이터는 로그에 저장하지 않고, 상태와 선택값 중심으로 추적하도록 정리

Verification:
- `pages/survey.html` 내 `<script>` 추출 후 JavaScript 파싱 확인
- 결과 페이지 직접 진입에서 `result_page_view`와 결과 섹션 로깅 구조 확인 예정
...
Purpose:
YUNN MVP 행동 로그를 운영자가 GA4에서 볼 수 있도록 Google Tag Manager 연결 구조를 추가했다. 실제 GTM Container ID가 아직 제공되지 않았기 때문에 안전한 placeholder 상태로 두고, ID 입력 시 즉시 GTM script가 로드되도록 구성했다.
...
Main implementation:
- 모든 HTML 진입 페이지에 GTM loader scaffold 추가
- `window.YUNN_GTM_ID`가 비어 있으면 GTM 요청을 보내지 않고 `window.yunnGtmStatus = 'missing_container_id'`로 표시
- 실제 `GTM-XXXXXXX` ID 입력 시 `https://www.googletagmanager.com/gtm.js`를 비동기 로드
- `trackYunnEvent()` payload에 GTM Custom Event trigger용 `event` 필드 추가
- 기존 로컬/백엔드 호환을 위해 `event_name` 필드는 유지
- GTM/GA4 태그, 변수, custom dimensions, MVP funnel 설정 문서 추가

Verification:
- HTML 내 스크립트 JavaScript 파싱 확인 예정
- `window.dataLayer`에 `event`와 `event_name`이 함께 들어가는지 확인 예정
...
Purpose:
Google Tag Manager에서 발급된 실제 컨테이너 ID `GTM-P2NX3N5K`를 YUNN 웹페이지 전체에 적용하고, GTM 권장 설치 방식에 맞춰 `<noscript>` fallback도 추가했다.
...
Main implementation:
- `window.YUNN_GTM_ID` 값을 `GTM-P2NX3N5K`로 설정
- 모든 HTML 페이지의 `<body>` 바로 뒤에 GTM noscript iframe 추가
- `docs/GTM-GA4-SETUP.md`를 실제 컨테이너 ID 기준으로 업데이트

Verification:
- HTML 내 스크립트 JavaScript 파싱 확인 예정
- 로컬 브라우저에서 `window.yunnGtmStatus === 'loading'` 및 GTM script URL 확인 예정

### GA4 MVP 핵심 KPI 이벤트 추가

Purpose:
GTM/GA4 연결 완료 후 운영자가 MVP 퍼널을 바로 볼 수 있도록 대표 이벤트 `survey_start`, `survey_complete`, `result_view`를 `pages/survey.html`의 실제 사용자 흐름에 추가했다.
...
Main implementation:
- 설문 진입 시 `survey_start` 발화
- 진단 완료 후 분석/로딩 화면 진입 시 `survey_complete` 발화
- 결과 화면 렌더링 시 `result_view` 발화
- 기존 세부 이벤트인 `landing_cta_click`, `analysis_completion_success`, `result_page_view`는 유지
- GA4에서 가장 먼저 볼 퍼널을 `survey_start -> survey_complete -> result_view`로 문서화
