...
function persistLog(payload: Record<string, unknown>) {
  try {
    ...
    events.push(payload)
    window.localStorage.setItem(ANALYTICS_LOG_KEY, JSON.stringify(events.slice(-ANALYTICS_LOG_MAX)))
  } catch {
    // 분석 로그 저장은 best-effort — 절대 사용자 흐름을 막지 않는다.
  }
}

function emit(eventName: string, sessionId: string, properties: Record<string, unknown>) {
  const payload = {
    event: eventName,
...
export function trackYunnEvent(eventName: string, properties: Record<string, unknown> = {}) {
...
export function trackCategoryOptionClick(category: string, step: RoutineStepId) {
  return trackYunnEvent('category_option_click', { category, step })
}

export function trackIngredientResultView(mismatchCount: number) {
  return trackYunnEvent('ingredient_result_view', { mismatch_count: mismatchCount })
}
