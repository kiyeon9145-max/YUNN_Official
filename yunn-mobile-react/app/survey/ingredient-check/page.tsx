...
import RoutineInputStep, { type ProductEntry, type RoutinePeriod } from '../components/routine-input-component'
import { usePageTracking } from '../../lib/use-page-tracking'
import { trackCategoryOptionClick, trackRoutineStepComplete, type PageId, type RoutineStepId } from '../../lib/analytics'
...
export default function IngredientCheckPage() {
  ...
  const trackNewRow = (prev: ProductEntry[], next: ProductEntry[], routineStep: RoutineStepId) => {
    ...
    <div className="mx-auto min-h-screen max-w-phone-max bg-white px-shell-x pt-8 pb-10">
      ...
      {step === 'done' && (
        <div className="pt-24 text-center">
          ...
          <p className="text-sm text-ink-faint">성분 분석 결과 화면은 다음 단계에서 만들 예정이에요.</p>
        </div>
      )}
    </div>
  )
