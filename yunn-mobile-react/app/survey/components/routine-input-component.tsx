...
const CATEGORIES = ['클렌저', '토너', '세럼', '모이스처라이저', '선크림', '기타'] as const
...
export default function RoutineInputStep({
  ...
  const handleNameChange = (index: number, name: string) => {
    onChange(value.map((entry, i) => (i === index ? { ...entry, name } : entry)))
  }
  ...
  const nextDisabled = (requiresAtLeastOne && value.length === 0) || hasIncompleteRow
    ...
    <>
      ...
      <div className="mb-5 flex flex-col gap-5">
        {value.map((row, index) => (
          <div key={index}>
            <div className="mb-2">
              <RoutineTag label={row.category} onRemove={() => handleRemove(index)} />
            </div>
...
function RoutineTag({ label, onRemove }: { label: string; onRemove: () => void }) {
  return (
...
function CategorySelect({ onSelect }: { onSelect: (category: string) => void }) {
  ...
  return (
    <div className="relative">
      ...
      >
        <span>화장품 종류 선택</span>
        <i className={`ph ph-caret-down text-[14px] transition-transform ${open ? 'rotate-180' : ''}`}></i>
      </button>
      {open && (
