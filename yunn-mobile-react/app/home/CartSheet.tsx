...
export default function CartSheet({ open, count, item, onClose, onViewCart }: CartSheetProps) {
  return (
    <aside
    ...
    >
      ...
      <div className="grid grid-cols-[116px_1fr] gap-[14px] items-start pb-[20px]">
        ...
        <div>
          ...
          <div className="text-[14px] leading-[19px] mb-[11px]">{item?.name ?? ''}</div>
          <div className="flex items-center gap-[8px] text-[12px]">
            <span>Qty 1</span>
            <span>{item?.price ?? ''}</span>
            {item?.discount && (
              <span className="text-[#E5484D] font-semibold">{item.discount}</span>
            )}
            {item?.originalPrice && (
              <span className="text-[#CCCCCC] line-through">{item.originalPrice}</span>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="border-t border-[#E5E5E5] pt-[10px]">
        <div className="flex justify-between items-center px-[5px] pb-[15px] text-[14px]">
          <span>Cart <span>({String(count).padStart(2, '0')} items)</span></span>
